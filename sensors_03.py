...
def get_sensors_data(sense):  # noqa: F811
    """Collects a specific sensor measure from the Sense HAT.

    Args:
        sense (str): name of the sensor to read

    Returns:
        JSON dictionary: sensor name key and measure value
    """
    if sense == "temperature":
        return json.dumps({"temperature": _estimate_temp()}, indent=2)
    elif sense == "humidity":
        return json.dumps({"humidity": _read_sense_humidity()}, indent=2)
    elif sense == "pressure":
        return json.dumps({"pressure": _read_sense_pressure()}, indent=2)
    else:
        return json.dumps({"message": "sensor not found!"}, indent=2), 404
