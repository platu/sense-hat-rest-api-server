...
def test_successful_delete_with_color(client):
    response = client.delete("/api/v1/leds/", json={"color": [0, 64, 0]})
    assert response.status_code == 200  # nosec B101
    assert '{"message": "LEDs cleared"}' in (
        response.data.decode())  # nosec B101
...
def test_failed_delete_invalid_json(client):
    response = client.delete("/api/v1/leds/", data="invalid json")
...
def test_failed_delete_invalid_color(client):
    response = client.delete("/api/v1/leds/", json={"color": "invalid color"})
    assert response.status_code == 400  # nosec B101
...
def test_failed_delete_invalid_color_value(client):
    response = client.delete("/api/v1/leds/", json={"color": [0, 64, 256]})
    assert response.status_code == 400  # nosec B101
...
def test_successful_get(client):
    ...
    assert all(len(color) == 3 for color in data["leds"])  # nosec B101
    assert all(all(c >= 0 and c <= 255 for c in color)
               for color in data["leds"])  # nosec B101
...
def test_successful_post(client):
    ...
    response = client.post("/api/v1/leds/", json=data)
    assert response.status_code == 200  # nosec B101
...
def test_failed_post_invalid_json(client):
    response = client.post("/api/v1/leds/", data="invalid json")
    assert response.status_code == 400  # nosec B101
    assert '{"error": "Invalid JSON"}' in (
        response.data.decode())  # nosec B101
...
def test_failed_post_missing_leds(client):
    response = client.post("/api/v1/leds/", json={"notleds": "missing"})
    assert response.status_code == 400  # nosec B101
...
def test_failed_post_invalid_color(client):
    response = client.post(
        "/api/v1/leds/", json={"leds": [[0, 0, 96], [47, 68, 256]]})
    assert response.status_code == 400  # nosec B101
    assert '{"error": "Invalid color value"}' in (
        response.data.decode())  # nosec B101
...
def test_failed_post_invalid_color_matrix_size(client):
    response = client.post(
        "/api/v1/leds/", json={"leds": [[0, 0, 96], [47, 68, 255]]})
    assert response.status_code == 400  # nosec B101
